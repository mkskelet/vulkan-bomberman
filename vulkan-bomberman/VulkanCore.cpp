...
void VulkanRenderer::CreateTextureImage(const char* path, VkImage* image, VkDeviceMemory* imageMemory, bool useMipMaps, uint32_t* mipLevels)
{
...
void VulkanRenderer::CreateTextureImageView(VkImageView* imageView, VkImage* image, uint32_t mipLevels)
{
	*imageView = CreateImageView(*image, VK_FORMAT_R8G8B8A8_UNORM, VK_IMAGE_ASPECT_COLOR_BIT, mipLevels);
}

void VulkanRenderer::CreateTextureSampler(VkSampler* sampler, uint32_t mipLevels)
{
...
void VulkanRenderer::CreateGraphicsPipeline(int& index, const char* vertexShaderPath, const char* fragmentShaderPath)
{
	// load shaders
...
void VulkanRenderer::UpdateDescriptorSets(std::vector<VkDescriptorSet>* descriptorSets, VkImageView* image, VkSampler* sampler)
{
	for (size_t i = 0; i < swapChainImages.size(); i++)
	{
...
VkImageView VulkanRenderer::CreateImageView(VkImage image, VkFormat format, VkImageAspectFlags aspectFlags, uint32_t mipLevels)
{
	...
	if (auto r = vkCreateImageView(device, &viewInfo, nullptr, &imageView) != VK_SUCCESS)
	{
